  ...
  return <section className="order-detail-page" aria-labelledby="order-detail-title">
    ...
    {order && <>
      <section className="order-detail-card" data-testid="order-detail">
        ...
        <div className="order-facts"><div><span>Status</span><strong>{order.status}</strong></div><div><span>Currency</span><strong>{order.currency}</strong></div><div><span>Version</span><strong>{order.version}</strong></div><div><span>Created</span><strong>{new Date(order.created_at).toLocaleString()}</strong></div></div>
        <OrderSnapshot order={order} />
        {order.status === "pending_payment" && <div className="order-cancel-zone"><p>{activePayment ? "Payment is in progress. Cancel is unavailable until the current Payment Attempt finishes." : "Cancel is explicit. It releases the reservation and does not add items back to Cart."}</p>{!activePayment && <button className="danger-button" disabled={cancelMutation.isPending} onClick={() => cancelMutation.mutate()} type="button">{cancelMutation.isPending ? "Cancelling…" : "Cancel pending order"}</button>}</div>}
